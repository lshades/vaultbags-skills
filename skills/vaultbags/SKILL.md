...
| `/api/agent/strategies` | Every vault strategy: the four presets (Classic, Growth, Hard Money, Income) plus each distinct strategy a live project has frozen. A strategy is a signed selection of 2-4 certified real-world assets, each at least 15%, identified by the hash of that selection. Presets run as live paper strategies whose daily decisions are frozen and stamped on-chain before any project adopts them, so the track record is verifiable rather than narrated. |
| `/api/agent/strategy?strategy=<id-or-key>` | One strategy in depth: its signed asset selection with base weights, which tokens adopted it, and its recent daily decisions with per-day weights, convictions and on-chain memo receipts. Accepts a preset id (`classic`, `growth`, `hard-money`, `income`) or a 64-hex strategy key. |
...
Per-strategy decisions verify the same way: `GET /api/proof/strategy/<YYYY-MM-DD>/<preset-or-key>` returns the stored fields that strategy's receipt commits to (the raw signed selection, the day's per-mint weights and convictions), the payload built by the published rules, the stored hash, the reconstructed memo text (`vaultbags:strategy:v1:<date>:<key>:<hash>`) and the expected signer.
